...
class TestContactForm(TestCase):
    ...
    def test_confirm_field_is_required(self):
        """
        Test that the confirm field is required by leaving it blank
          but completing other required fields
        The test will verify that the error message is displayed.
        The test asserts whether or not there's a name key in the
          dictionary of form errors.
        Test that the confirm field is required.
        """
        form = BookingForm(
            {'booking_date': '2022-09-14',
             'booking_time': '17:00',
        ...
        self.assertIn('confirm', form.errors.keys())
        self.assertEqual(form.errors['confirm'][0], 'This field is required.')
    ...
    def test_only_required_fields_are_in_form_metaclass(self):
        ...
        Test to ensure that the only necessary fields are displayed in the form
          ie. no fields were added that should not be visible/editable to the user.
